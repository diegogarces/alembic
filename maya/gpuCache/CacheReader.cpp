...
// TBB will implement std::condition_variable if the compiler doesn't support it.
// Unfortunately, TBB's std::condition_variable is slightly different from the standard.
// 1) C++0x's condition_variable takes std::mutex as its argument. We should use the
//    template version std::condition_variable_any instead since we are using tbb::mutex.
//    We typedef TBB's std::condition_variable to std::condition_variable_any.
// 2) C++0x's condition_variable::wait_for() takes std::chrono while TBB is using 
//    tbb::tick_count.
//    We define a macro to workaround this.
#if TBB_IMPLEMENT_CPP0X == 0
    #include <condition_variable>
    #include <mutex>
    #define DURATION_SECONDS(x) (std::chrono::seconds(x))
#else
    namespace std {
...
public:
    ...
    public:
        ...
        virtual task* execute()
        {
            ...
            try {
                ...
                if (cacheReader && cacheReader->valid()) {
                    ...
                    geometry = cacheReader->readHierarchy(
                        validatedGeometryPath, !Config::isIgnoringUVs(), pivots);
            ...
            fScheduler->hierarchyTaskFinished(fShapeNode, 
                geometry, validatedGeometryPath, materials, pivots, fProxy);
            fProxy.reset();
    ...
    public:
        ...
        Pivots::Ptr& pivots()                  { return fPivots;}
...
private:
                               ...
                               CacheReaderProxy::Ptr& proxy)
    {
        ...
        fTaskRunning->finishTask(geometry, validatedGeometryPath, materials, pivots);
                           ...
                           const MString&   geometryPath)
    {
        ...
        Pivots::Ptr noPivots;
        fTaskRunning->finishTask(geometry, geometryPath, noMaterials, noPivots);
...
private:
    tbb::mutex                   fBigMutex;
    std::condition_variable_any  fCondition;
    ...
    std::condition_variable_any fPauseCond;
};
...
{
    return fScheduler->pullHierarchy(node, geometry, validatedGeometryPath, materials, pivots);
}
