...
public:
    ...
    // Pointer to a mutable sub node
    typedef boost::shared_ptr<Pivots>       MPtr;

    // Pointer to an immutable sub node
    typedef boost::shared_ptr<const Pivots> Ptr;

    // Weak pointer to an immutable sub node
    typedef boost::weak_ptr<const Pivots>   WPtr;

    const MPoint& getScalePivot() const              { return scalePivot; }
    const MVector& getScalePivotTranslation() const     { return scalePivotTranslation; }
    const MPoint& getRotatePivot() const { return rotatePivot; }
    const MVector& getRotatePivotTranslation() const { return rotatePivotTranslation;}

    static MPtr create(const MPoint& _scalePivot, const MVector& _scalePivotTranslation, const MPoint& _rotatePivot, const MVector& _rotatePivotTranslation) {
            return boost::make_shared<Pivots>(_scalePivot, _scalePivotTranslation, _rotatePivot, _rotatePivotTranslation);
    }
...
private:
    ...
    Pivots(const MPoint& _scalePivot, const MVector& _scalePivotTranslation, const MPoint& _rotatePivot, const MVector& _rotatePivotTranslation):
        scalePivot(_scalePivot), scalePivotTranslation(_scalePivotTranslation), rotatePivot(_rotatePivot), rotatePivotTranslation(_rotatePivotTranslation)
    {}

    MPoint scalePivot;
    MVector scalePivotTranslation;
    MPoint rotatePivot;
    MVector rotatePivotTranslation;
};
